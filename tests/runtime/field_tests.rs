...
use tsonic_rust_runtime::{
    Field, FieldKey, ObjectHandle, ReadField, ReadFieldOf, WriteField, WriteFieldOf,
};
...
fn update<Owner, Key, Error>(
    owner: &Owner,
    key: &Key,
    change: impl FnOnce(Owner::Output) -> Owner::Output,
) -> Result<(), Error>
where
...
fn dependent_fields_preserve_inline_storage_and_exact_integer_width() {
    ...
    assert_eq!(
        core::mem::size_of::<Cell<u64>>(),
        core::mem::size_of::<u64>()
    );
    let value = Record {
        count: Cell::new(9_007_199_254_740_993),
        label: "value".into(),
    };
...
fn dependent_fields_preserve_shared_aliases_and_distinct_result_types() {
    let value = ObjectHandle::new(Record {
        count: Cell::new(7),
        label: "retained".into(),
    });
    ...
    assert_eq!(
        ReadField::<Count, &'static str>::read_field(&alias, &Count::default()),
        Ok(11)
    );
    assert_eq!(
        ReadField::<Label, Infallible>::read_field(&alias, &Label::default()),
        Ok("retained".into())
    );
...
fn dependent_fields_propagate_read_and_write_failures_without_mutating() {
    let missing = ObjectHandle::new(Record {
        count: Cell::new(0),
        label: "missing".into(),
    });
    assert_eq!(
        update::<_, _, &'static str>(&missing, &Count::default(), |_| panic!("read failed")),
        Err("missing")
    );
    let value = ObjectHandle::new(Record {
        count: Cell::new(7),
    ...
    assert_eq!(
        WriteField::<Count, &'static str>::write_field(&value, &Count::default(), 0),
        Err("zero")
    );
