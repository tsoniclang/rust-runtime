...
fn bound_field_copy_address_and_write_do_not_read_the_pointer() {
    ...
    let pointer = Location::<i32, Infallible>::try_bind(ObjectIdentity::new(),
        move || { reader_count.set(reader_count.get() + 1); Ok(reader.get()) },
        move |next| { writer.set(next); Ok(()) });
    let owner = Location::<_, Infallible>::allocate(Record { count: RecordField::Bound(pointer.clone()), visits: 0 });
    let copied = owner.try_load().unwrap();
    let returned = Location::<_, Infallible>::allocate(copied);
    let address = RecordField::location_from_value(&returned, "count".into(),
        |record| &record.count, |record| &mut record.count).unwrap();
    assert!(Location::same(Some(&address), Some(&pointer)));
    assert_eq!(Location::hash(Some(&address)), Location::hash(Some(&pointer)));
...
fn ordinary_record_copies_remain_independent_and_locations_remain_parent_relative() {
    let value = Record { count: RecordField::Value(3), visits: 0 };
    let mut copy = value.clone();
    copy.count.try_store(5).unwrap();
    assert_eq!(value.count.try_load(), Ok(3));
    let owner = Location::<_, Infallible>::allocate(value);
    let first = RecordField::location_from_value(&owner, "count".into(),
        |record| &record.count, |record| &mut record.count).unwrap();
    let alias = RecordField::location_from_value(&owner, "count".into(),
        |record| &record.count, |record| &mut record.count).unwrap();
    assert!(Location::same(Some(&first), Some(&alias)));
    owner.try_store(Record { count: RecordField::Value(11), visits: 0 }).unwrap();
...
fn reference_bound_callbacks_run_outside_the_record_borrow() {
    let owner = ObjectHandle::new(Record { count: RecordField::Value(0), visits: 0 });
    let reader = owner.clone();
    let writer = owner.clone();
    let pointer = Location::try_bind(ObjectIdentity::new(),
        move || { reader.with_mut(|record| record.visits += 1); Ok(19) },
        move |_| { writer.with_mut(|record| record.visits += 1); Ok(()) });
    owner.with_mut(|record| record.count = RecordField::Bound(pointer.clone()));
    let address = RecordField::location_from_object(&owner, "count".into(),
        |record| &record.count, |record| &mut record.count);
    assert!(Location::same(Some(&address), Some(&pointer)));
    assert_eq!(owner.with(|record| record.visits), 0);
    assert_eq!(RecordField::try_load_object(&owner, |record| &record.count), Ok(19));
    RecordField::try_store_object(&owner, |record| &record.count, |record| &mut record.count, 23).unwrap();
...
fn bound_fields_preserve_the_exact_error_and_retained_owner() {
    ...
    let pointer = Location::try_bind(ObjectIdentity::new(),
        move || { retained.set(retained.get() + 1); Err::<i32, _>(Rc::clone(&read_failure)) },
...
fn replacing_a_bound_field_does_not_retarget_previously_taken_addresses() {
    ...
    let owner = ObjectHandle::new(Record { count: RecordField::Bound(first.clone()), visits: 0 });
    let old = RecordField::location_from_object(&owner, "count".into(), |record| &record.count, |record| &mut record.count);
    owner.with_mut(|record| record.count = RecordField::Bound(second.clone()));
    let next = RecordField::location_from_object(&owner, "count".into(), |record| &record.count, |record| &mut record.count);
